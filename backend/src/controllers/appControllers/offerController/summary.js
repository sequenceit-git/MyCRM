...
const summary = async (req, res) => {
  try {
    ...
    let totalOffers = response[0].totalOffer[0] ? response[0].totalOffer[0].total : 0;
    let totalCount = response[0].totalOffer[0] ? response[0].totalOffer[0].count : 0;
    ...
    return res.status(200).json({
      ...
      message: 'Successfully fetched offer summary statistics',
    });
  } catch (error) {
